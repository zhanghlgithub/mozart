...
#undef LAPSULE_AUTOPLAY

#ifdef LAPSULE_AUTOPLAY
...
bool lapsule_is_controler(void)
{
#if (SUPPORT_DMR == 1)
	control_point_info *info = mozart_render_get_controler();

	if (info && info->music_provider && strcmp(LAPSULE_PROVIDER, info->music_provider) == 0)
		return true;
	else
		return false;
#else
...
//		printf("read file ok! %s\n",buff);
		...
		const char *result = json_object_get_string(tmp);
